...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $content = $name . ' просит связаться с ним по номеру телефона:' . $phone . ' в ' . $time;

    $headers  = 'MIME-Version: 1.0' . "\r\n";
    $headers .= 'Content-type: text/html; charset=utf-8' . "\r\n";

    // Первый параметр - кому отправляем письмо, второй - тема письма, третий - содержание
    $success = mail("vodomistr@gmail.com", 'Просьба перезвонить!', $content, $headers);
    ...
    echo "This request method is not supported by the server";
}